...
- Backend chỉ kiểm tra quyền entitlement và tự động sinh **Signed Download URL** hoặc Pre-signed URL (chẳng hạn S3), trả về cho client chứ không tự stream binary file.
- Nếu chính sách là single-use thì `issued -> consumed` phải atomic để chống double-click/race. Cần phân định giữa TTL của URL và TTL của lượt sử dụng (nếu có).
- `consumed` không có nghĩa file tải chắc chắn hoàn tất 100%, nhưng đủ để xem token đã bị dùng (thường callback từ CDN).
...
- Đối với thanh toán online đơn hàng physical, bắt buộc khởi tạo `expires_at` dựa trên **Pending Hold TTL** để khóa hàng có thời hạn trong `inventory_reservations`.
- `release_timeout` do Scheduler/Worker phát hiện `now > expires_at` (TTL timeout) nếu thanh toán bị treo hoặc gateway không callback.
- Redis stock guard chỉ là pre-check performance trong flash sale, không thay inventory transaction tại PostgreSQL.
