...
| Cột                  | Kiểu dữ liệu | Null  | Mô tả                          | Ghi chú                     |
| -------------------- | ------------ | ----- | ------------------------------ | --------------------------- |
| `id`                 | bigint       | không | định danh session              | PK, BIGSERIAL               |
| `user_id`            | bigint       | không | FK tới `users`                 | ON DELETE CASCADE           |
| `device_id`          | bigint       | không | FK tới `user_devices`          | ON DELETE CASCADE           |
| `refresh_token_hash` | text         | không | hash refresh token             | UNIQUE, không lưu plaintext |
| `session_status`     | varchar(20)  | không | `active`, `revoked`, `expired` | canonical                   |
| `expires_at`         | timestamptz  | không | thời điểm session hết hạn      |                             |
| `ip_address`         | varchar(45)  | có    | IP tạo session                 | nhạy cảm vận hành           |
| `user_agent`         | text         | có    | user agent tại login           |                             |
| `last_seen_at`       | timestamptz  | không | lần hoạt động cuối             | default NOW()               |
| `revoked_at`         | timestamptz  | có    | thời điểm revoke               |                             |
| `revoked_reason`     | varchar(255) | có    | lý do revoke                   |                             |
| `created_at`         | timestamptz  | không | audit create                   | default NOW()               |
| `updated_at`         | timestamptz  | không | audit update                   | default NOW()               |

### Constraints và indexes

- PK: `id`
- UNIQUE: `refresh_token_hash`
- UNIQUE: `user_id, device_id` (Đảm bảo 1 thiết bị của 1 user chỉ duy trì đúng 1 Session)
- INDEX: `idx_user_sessions_id_status` (user_id, session_status)
- INDEX: `idx_user_sessions_expires_at` (expires_at)

### Quy tắc update

- `refresh_token_hash` không rotate tùy tiện ngoài flow refresh.
- `session_status` chỉ đi qua auth state policy.
...
- Định danh độc lập (không đa hình) cho tất cả các item có thể bán (physical book, ebook_format, membership plan) để giải quyết bài toán toàn vẹn tham chiếu (referential integrity) của Cart và Order.

| Cột               | Kiểu dữ liệu | Null  | Mô tả                                                    |
| ----------------- | ------------ | ----- | -------------------------------------------------------- |
| `sellable_sku_id` | bigint       | không | PK                                                       |
| `sku_type`        | varchar(32)  | không | `ebook`, `physical`, `membership_plan`                   |
| `reference_id`    | bigint       | không | trỏ tới ID thật (book_format_id hoặc membership_plan_id) |
| `is_active`       | boolean      | không | trạng thái có bán hay không                              |
| `created_at`      | timestamptz  | không |                                                          |
| `updated_at`      | timestamptz  | không |                                                          |
...
| `sellable_sku_id`              | bigint       | không | định danh SKU chuẩn hóa  |
...
| `sellable_sku_id`      | bigint       | không | định danh SKU (tham chiếu bảng sellable_skus) |
...
| `expires_at`               | timestamptz  | có    | timeout reservation (Pending Hold TTL)                    |
...
| `ip_address`      | varchar(45)  | có    | nếu có                    |
| `created_at`      | timestamptz  | không |                           |
